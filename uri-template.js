...
export const replace = (str, map) =>
  str.replace(regex, (_, operator, variableList) => {
    ...
    return (
      ...
      values
        .map(([name, value]) =>
          value ? (named ? name + (value ? "=" + value : ifemp) : value) : "",
        )
        .filter((x) => x)
        .join(sep)
    );
...
export const getVariables = (str) =>
  new Set(
    Array.from(str.matchAll(regex), ([, , variableList]) =>
      variableList.split(",").map((varspec) => varspec.match(varspecRegex)?.[1]),
    ).flat(),
  );

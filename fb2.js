...
class FB2Converter {
  constructor(fb2) {
    ...
    this.bins = new Map(Array.from(this.fb2.getElementsByTagName("binary"), (el) => [el.id, el]));
  }
  getImageSrc(el) {
  ...
  anchor(node) {
    ...
    if (node.getAttribute("type") === "note") el.setAttributeNS(NS.EPUB, "epub:type", "noteref");
    return el;
  }
  convert(node, def) {
  ...
  const sectionData = bodyData[0][0]
    ...
    .map(({ el, ids }, id) => {
      ...
      const titles = Array.from(el.querySelectorAll(":scope > section > .title"), (el, index) => {
        ...
        const size =
          new TextEncoder().encode(section.innerHTML).length -
          Array.from(section.querySelectorAll("[src]")).reduce(
            (sum, el) => sum + (el.getAttribute("src")?.length ?? 0),
            0,
          );

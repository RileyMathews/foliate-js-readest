...
const animateScroll = (element, scrollProp, startValue, endValue, duration, extraElements = []) =>
  new Promise((resolve) => {
    if (document.hidden) {
...
class View {
    ...
    return new Promise((resolve) => {
      this.#iframe.addEventListener(
        ...
        () => {
          ...
          const bgUrl = this.docBackground?.match(/url\(["']?([^"')]+)["']?\)/)?.[1];
          if (bgUrl) {
            const img = new Image();
            img.onload = () => {
              this.#bgImageSize = {
  ...
  columnize({
    ...
    setStylesImportant(doc.documentElement, {
      ...
      "column-gap": vertical
        ? `${(marginTop + marginBottom) * 1.5}px`
        : `${gap + marginRight / 2 + marginLeft / 2}px`,
      "column-fill": "auto",
    ...
    if (this.#column) {
      ...
      if (this.#bgImageSize) {
        ...
        if (crossSize > 0) {
          ...
          const scaledSize = this.#vertical ? (imgW * crossSize) / imgH : (imgH * crossSize) / imgW;
  ...
  showLoupe(winX, winY, { isVertical, color, gap, margin, radius, magnification }) {
    ...
    if (!this.#loupeEl || !this.#loupeEl.isConnected) {
      ...
      if (this.#vertical) htmlWrapper.style.height = `${doc.documentElement.scrollHeight}px`;
      else htmlWrapper.style.width = `${doc.documentElement.scrollWidth}px`;
  ...
  hideLoupe() {
    ...
    if (this.#overlayer) this.#overlayer.clearHole();
  }
  destroyLoupe() {
    if (this.#loupeEl) {
      this.#loupeEl.remove();
...
export class Paginator extends HTMLElement {
  ...
  constructor() {
    ...
    const debouncedScroll = debounce(() => {
      if (this.scrolled && !this.#isAnimating) {
        ...
        if (
          !this.noPreload &&
          !this.noContinuousScroll &&
          !this.#filling &&
          this.#renderedStart < this.size
        ) {
          ...
          if (firstIndex != null) {
            ...
            if (prevIdx != null && !this.#views.has(prevIdx)) {
              this.#filling = true;
              this.#loadAdjacentSection(prevIdx).finally(() => {
                this.#filling = false;
              });
            }
          }
    ...
    this.#container.addEventListener("scroll", () => {
      ...
      if (!this.noPreload && !this.noContinuousScroll && !this.#filling && !this.#stabilizing) {
        const minPages = 5;
        const pagesAhead =
          this.size > 0 ? Math.floor((this.#renderedViewSize - this.#renderedEnd) / this.size) : 0;
        if (pagesAhead < minPages) {
          ...
          if (lastIndex != null) {
            ...
            if (nextIdx != null && !this.#views.has(nextIdx)) {
              this.#filling = true;
              this.#loadAdjacentSection(nextIdx).finally(() => {
                this.#filling = false;
              });
            }
          }
  ...
  #replaceBackground(atPosition) {
    ...
    if (this.scrolled) {
      ...
      for (const [, view] of this.#sortedViews) {
        ...
        const isTransparent =
          !resolved || /^\s*(transparent|rgba\(0,\s*0,\s*0,\s*0\))/.test(resolved);
  ...
  #beforeRender({ vertical, rtl }) {
    ...
    const divisor = Math.min(
      ...
      Math.ceil(Math.floor(size) / Math.floor(maxInlineSize)),
    );
  ...
  scrollBy(dx, dy) {
    ...
    this.containerPosition = Math.max(min, Math.min(max, this.containerPosition + delta));
  }
  ...
  #onTouchMove(e) {
    ...
    if (
      ...
      !this.hasAttribute("eink") &&
      (!isStylus || Math.abs(dx) > 1)
    ) {
      this.scrollBy(dx, 0);
    } else if (
      this.#vertical &&
  ...
  #getRectMapper(view) {
    if (this.scrolled) {
      ...
      return this.#vertical
        ? ({ left, right }) => ({
            left: size - right - marginTop,
            right: size - left - marginBottom,
          })
  ...
  async #scrollTo(offset, reason, smooth) {
    ...
    if (
      ...
      return animateScroll(this.#container, this.scrollProp, startPosition, offset, 300).then(
        () => {
  ...
  async #scrollToAnchor(anchor, reason = "anchor", smooth = false) {
    ...
    if (rects) {
      ...
      const rect =
        Array.from(rects).find((r) => r.width > 0 && r.height > 0 && r.x >= 0 && r.y >= 0) ||
        rects[0];
      if (!rect) return;
    ...
    if (this.scrolled) {
      ...
      const primarySize = primaryView
        ? primaryView.element.getBoundingClientRect()[this.sideProp]
        : this.#renderedViewSize;
    ...
    if (this.scrolled) {
      ...
      for (const [index, v] of this.#sortedViews) {
        ...
        const range = getVisibleRange(
          v.document,
          this.#renderedStart - off,
          this.#renderedEnd - off,
  ...
  async #preloadNext() {
    ...
    try {
      ...
      while (this.#views.size < maxSections && iterations < maxSections) {
        iterations++;
        const pagesAhead =
          size > 0 ? Math.floor((this.#renderedViewSize - this.#renderedEnd) / size) : 0;
  ...
  #afterScroll(reason) {
    ...
    if (this.scrolled) {
      ...
      detail.fraction =
        primarySize > 0
          ? Math.max(0, Math.min(1, (this.#renderedStart - primaryOffset) / primarySize))
          : 0;
  ...
  async #display(promise) {
    ...
    if (src) {
      ...
      const afterLoad = (doc) => {
        if (doc.head) {
          ...
          this.sections[index].spineProperties?.forEach((prop) =>
            doc.documentElement.setAttribute("data-" + prop, ""),
          );
    ...
    const resolvedAnchor =
      (typeof anchor === "function" ? anchor(primaryView.document) : anchor) ?? 0;
    ...
    this.#fillPromise = this.#fillVisibleArea({ reanchor: !this.scrolled });
    ...
    try {
      ...
      const afterLoad = (doc) => {
        if (doc.head) {
          ...
          const $style = doc.createElement("style");
          doc.head.append($style);
          section.spineProperties?.forEach((prop) =>
            doc.documentElement.setAttribute("data-" + prop, ""),
          );
          this.#styleMap.set(doc, [$styleBefore, $style]);
        }
  ...
  async #goTo({ index, anchor, select }) {
    if (this.#views.has(index)) {
      ...
      const onLoad = (detail) => {
        if (oldIndex >= 0 && !this.#views.has(oldIndex)) this.sections[oldIndex]?.unload?.();
        this.setStyles(this.#styles);
    ...
    if (this.scrolled) {
      if (this.#renderedStart > 0)
        return this.#scrollTo(
          Math.max(0, this.#renderedStart - (distance ?? this.size)),
          null,
          true,
        );
      return !this.atStart;
  ...
  #scrollNext(distance) {
    if (this.#views.size === 0) return true;
    if (this.scrolled) {
      if (this.#renderedViewSize - this.#renderedEnd > 2)
        return this.#scrollTo(
          Math.min(
            this.#renderedViewSize,
            distance ? this.#renderedStart + distance : this.#renderedEnd,
          ),
          null,
          true,
        );
  ...
  get atStart() {
    ...
    if (this.scrolled)
      return this.#adjacentIndex(-1, firstIndex) == null && this.#renderedStart <= 0;
  ...
  get atEnd() {
    ...
    if (this.scrolled)
      return (
        this.#adjacentIndex(1, lastIndex) == null && this.#renderedViewSize - this.#renderedEnd <= 2
      );
    return (
      this.#adjacentIndex(1, lastIndex) == null && this.#renderedPage >= this.#renderedPages - 1
    );
  }
  ...
  showLoupe(winX, winY, { isVertical, color, gap, margin, radius, magnification }) {
    this.#primaryView?.showLoupe(winX, winY, {
      isVertical,
      color,
      gap,
      margin,

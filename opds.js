...
export const getPublication = (entry) => {
  ...
  return {
    metadata: {
      ...
      content: getContent(children.find(filter("content")) ?? children.find(filter("summary"))),
      [SYMBOL.CONTENT]: getContent(
        children.find(filter("content")) ?? children.find(filter("summary")),
      ),
    },
    links,
...
export const getFeed = (doc) => {
  ...
  for (const entry of entries) {
    ...
    const isPub = [...linksByRel.keys()].some(
      (rel) => rel?.startsWith(REL.ACQ) || rel === "preview",
    );
...
export const getOpenSearch = (doc) => {
  ...
  const $url =
    $$urls.find((url) => isOPDSCatalog(url.getAttribute("type"))) ??
    $$urls.find((url) => isOPDSSearch(url.getAttribute("type"))) ??
    $$urls[0];

...
export class FixedLayout extends HTMLElement {
  ...
  #render(side = this.#side) {
    ...
    const transform = ({ frame, styles }) => {
      ...
      const sectionIndex =
        iframe.dataset.sectionIndex != null ? parseInt(iframe.dataset.sectionIndex) : undefined;
    ...
    for (const frame of showingFrames) {
      ...
      const index =
        frame.iframe.dataset.sectionIndex != null
          ? parseInt(frame.iframe.dataset.sectionIndex)
          : undefined;
  ...
  #evictScrollPages() {
    ...
    loaded.sort((a, b) => Math.abs(a.index - currentIndex) - Math.abs(b.index - currentIndex));
    ...
    if (cached && cached !== "loading") {
      if (cached.center) {
        ...
        await this.#showSpread({
          center: { index: sectionIndex, src: cached.center },
          spreadIndex: index,
          side,
        });
      } else {
  ...
  #refreshOverlayerForFrame(frame) {
    ...
    const isVisible = frame.element?.parentNode && frame.element.style.visibility !== "hidden";
    if (!isVisible) return;
  ...
  getContents() {
    ...
    return Array.from(this.#root.querySelectorAll("iframe"))
      ...
      .map((frame) => {
        const index =
          frame.dataset.sectionIndex != null ? parseInt(frame.dataset.sectionIndex) : undefined;
        return {

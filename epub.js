...
const getMetadata = (opf) => {
  ...
  const baseLang =
    $metadata.getAttribute("xml:lang") ?? opf.documentElement.getAttribute("xml:lang") ?? "und";
  ...
  const dc = Object.fromEntries(
    Object.entries(Object.groupBy(els.dc || [], (el) => el.localName)).map(([name, els]) => [
      name,
      els.map(parse),
    ]),
  );
  ...
  const legacyMeta = Object.fromEntries(
    els.legacyMeta?.map((el) => [el.getAttribute("name"), el.getAttribute("content")]) ?? [],
  );
  ...
  const metadata = {
    ...
    belongsTo: {
      ...
      series:
        belongsTo.series?.map(makeSeries) ??
...
class Resources {
  constructor({ opf, resolveHref }) {
    ...
    this.cover =
      ...
      this.manifest.find((item) => item.id === "cover" && item.mediaType.startsWith("image")) ??
      this.manifest.find(
